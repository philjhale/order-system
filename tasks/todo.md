...
- [ ] 6. CI/CD skeleton (Azure AD app with two federated OIDC credentials — PR-triggered subject + main-branch subject, so every phase's PR CI authenticates, not just one branch — granted Contributor at subscription scope (not resource-group scope, since task 5's Terraform is what creates that group), IDs stored as repo secrets; single top-level orchestrating workflow (not one independent file per service — `needs:` can't cross separate workflow runs) that calls each affected service's reusable build/test/docker-build-push/terraform-plan-apply workflow as a job in the same run; when a PR touches two service Terraform folders, subscribing service's apply job declares `needs:` on topic-owning service's apply job, now enforceable since both jobs share one run)
...
- [ ] 10. Order Service Dockerfile, Terraform + deploy (`services/order-service/Dockerfile` + `services/order-service/infra/terraform/`: Container App image from shared ACR, min_replicas=1 (Order Service also consumes Service Bus events and has no HTTP-driven scale trigger between calls), SQL DB, owned topics: OrderCreated/OrderCancelled/OrderConfirmed; path-filtered CI/CD build/push/deploy job; job runs EF Core migrations, retry-wrapped against Serverless auto-pause, against the DB before the Container App revision goes live)
...
- [ ] 14. Inventory Service Dockerfile, Terraform + deploy (`services/inventory-service/Dockerfile` + `services/inventory-service/infra/terraform/`: Container App image from shared ACR, min_replicas=1 (no HTTP traffic to scale from zero on), SQL DB, subscriptions to OrderCreated/OrderCancelled, owned topics: InventoryReserved/InventoryFailed/InventoryReleased + Order's new subscriptions to them; path-filtered CI/CD build/push/deploy job runs retry-wrapped EF Core migrations before Container App revision goes live; Order's apply job `needs:` Inventory's apply job)
