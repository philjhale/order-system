...
- [x] 3. Messaging abstraction (IEventPublisher/IEventSubscriber w/ explicit abandon-for-redelivery outcome, Service Bus + in-memory impls; MaxDeliveryCount=10 + scheduled-redelivery delay so a short cross-topic race isn't dead-lettered) — generation time: ~10m
- [x] 4. Azure account configuration (confirm/select subscription, register required resource providers: Microsoft.App/ServiceBus/Sql/ContainerRegistry/Storage) + Terraform remote state bootstrap (storage account + container, local state, one-time manual apply). Also do task 6's app-registration + OIDC-credential bootstrap here, ahead of task-number order — task 5's AAD group membership needs its object id. — generation time: ~1h active (session spanned overnight waiting on user `az login`/subscription confirmation); review found 1 Critical (public-access default on state storage account) + 2 Important findings, all fixed
- [x] 5. Terraform shared foundation (resource group, Container Apps environment, Service Bus namespace at Standard SKU (mandatory — Basic has no topics/subscriptions or sessions), shared user-assigned identity for ACR pulls (granted AcrPull on the registry, attached per-Container-App in later tasks — ACR auth is per-app, not environment-wide), shared Azure Container Registry, AAD group as SQL AAD-admin (CI app registration added as a member) — passwordless data-plane auth for Service Bus + SQL replaces Key Vault entirely, so no Key Vault is provisioned) — generation time: ~20m; two review rounds found 0 Critical, 5 Important total (implicit security defaults, exclusive vs. additive group membership, missing prevent_destroy, missing output), all fixed; `terraform apply` run live (8 resources, uksouth)
- [x] 6. CI/CD skeleton (Azure AD app with two federated OIDC credentials — PR-triggered subject + main-branch subject, so every task's PR CI authenticates, not just one branch — granted Contributor + User Access Administrator at subscription scope (not resource-group scope, since task 5's Terraform is what creates that group; Contributor alone can't create role assignments), IDs stored as repo secrets; single top-level orchestrating workflow (not one independent file per service — `needs:` can't cross separate workflow runs) that always builds/tests `shared/` and calls each affected service's reusable build/test/docker-build-push/terraform-plan-apply workflow as a job in the same run; a `shared/**`-only change is treated as affecting all 4 services (forces every service job to run, not none, since every service embeds `shared/` by reference) rather than being silently skipped by the per-service path filter; when a PR touches two service Terraform folders, subscribing service's apply job declares `needs:` on topic-owning service's apply job, now enforceable since both jobs share one run) — generation time: ~30m; review found 1 Critical (missing top-level `permissions:` block would break every OIDC login) + 2 Important (docker-build-push had no build-only mode for PR runs; paths-filter checkout needed full history), all fixed; first live PR-triggered run then surfaced 2 more real gaps review couldn't have caught statically — GitHub's OIDC subject for this repo is ID-qualified (`repo:owner@id/repo@id:...`), not the plain form task 4's federated credentials used, and the CI app registration had zero Microsoft Graph permissions so `terraform plan` on task 5's `azuread_group` resources 403'd — both fixed live against the Azure AD app (user-approved) and confirmed by a subsequent green `ci.yml` run on the PR; per-service docker-build-push/terraform-plan-apply job calls deliberately deferred to tasks 10/14/17/19, once those services have a Dockerfile/infra/terraform to run against

## Phase 1 — Order Service
- [x] 7. Order domain + persistence (Orders/OrderItems/OrderEvents, state machine; DbContext uses EnableRetryOnFailure() for Azure SQL Serverless auto-pause resilience) — generation time: ~15m; review found 1 Important (Order.Create derived TotalAmount from item subtotals instead of taking it as client-submitted, per spec's "no catalog/pricing service in scope" known limitation), fixed
- [x] 8. Order Service HTTP API (POST /orders, GET /orders/{id}) — generation time: ~20m; review found 1 Important (duplicate ProductId in request items crashed SaveChangesAsync as an unhandled 500 instead of a 400), fixed
- [x] 9. Order Service event consumers (InventoryReserved/InventoryFailed/PaymentCompleted/PaymentFailed/OrderShipped/OrderDelivered → state transitions; InventoryReleased consumed + audit-logged, no transition; events arriving before their precondition state is reached are abandoned for redelivery, not rejected/dropped — relies on subscription max-delivery-count for poison messages) — generation time: ~15m; review found 1 Critical (redelivery of an already-applied PaymentCompleted/PaymentFailed skipped the follow-up OrderConfirmed/OrderCancelled publish entirely, permanently losing it if an earlier attempt committed the transition but then failed to publish) + 1 Important (InventoryReleased had no redelivery guard, so a redelivered message added a duplicate audit row), both fixed
- [x] 10. Order Service Dockerfile, Terraform + deploy (`services/order-service/Dockerfile` + `services/order-service/infra/terraform/`: own managed identity granted Azure Service Bus Data Owner on the namespace; SQL DB with Azure-AD-only auth + firewall rule allowing Azure services; Container App image from shared ACR, both ACR-pull and own identity attached, min_replicas=1 (Order Service also consumes Service Bus events and has no HTTP-driven scale trigger between calls); owned topics: OrderCreated/OrderCancelled/OrderConfirmed; path-filtered CI/CD build/push/deploy job; a `azurerm_container_app_job` (not the GitHub runner, which can't reach SQL) first creates the contained DB user via a CI-fetched short-lived AAD token, then runs retry-wrapped EF Core migrations, before the Container App revision goes live) — generation time: ~20m (initial build+review+fix cycle); review found 1 Critical (`az containerapp job start --env-vars` replaces rather than merges the container's env list, so the migration job lost its other env vars and failed immediately) + 2 Important (db_ddladmin shared by the always-running app's identity, not just the migration job; unguarded transient CLI failure could abort the whole polling loop), all fixed or documented as accepted MVP tradeoffs; not yet applied to real Azure (`terraform validate`/`fmt` and a local `docker build`-equivalent `dotnet publish` both verified; actual `terraform apply`/deploy happens via CI on merge, per this repo's established pattern); PR #18 follow-up (pre-merge, not live-tracked — commit span is a lower bound only, ~15-20m): `/agent-skills:shipping-and-launch` pre-launch review added a missing `/health` endpoint + Container App liveness/readiness probes; separately removed task-number references (e.g. "task 9") from code comments/docs repo-wide, added a CLAUDE.md convention against reintroducing them; `/agent-skills:code-simplify` pass hoisted duplicated CI env vars to job level, deduplicated a connection-string helper in Program.cs, and replaced two hardcoded Terraform values in ci.yml with proper outputs
